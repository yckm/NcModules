...
namespace Clash {
	...
	public:
		 ...
		 * @参数 mat 转换矩阵
		 */
		static void setMain2AssistMat(Eigen::Matrix4d mat);
	};
...
#pragma region 函数
	void DistanceDectector::setMain2AssistMat(Eigen::Matrix4d mat) {
		for (int i = 0; i < 4; i++) {
			for (int j = 0; j < 4; j++) {
				if (j == 3) {
					arm2TransMat(i, j) = mat(i, j)*1000;
				}
				else {
					arm2TransMat(i, j) = mat(i, j);
				}
			}
		}
	}
