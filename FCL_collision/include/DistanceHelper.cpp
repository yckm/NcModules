...
std::vector<std::string > Clash::DistanceHelper::sampleFclModelToJson(std::vector<fcl::CollisionObject<double>*> fclCollisionObjs, int pointCnt)
{
	...
	arr.push_back("\"pcds\":[");
	for (int j = 0; j < fclCollisionObjs.size(); j++) {
		arr.push_back("[");
		for (int i = 0; i < pointCnt; i++) {
			...
			double dist = fcl::distance<double>(fclCollisionObjs[j], ball, request, result);
			if (dist >= 0) {
...
std::vector<std::string > Clash::DistanceHelper::sampleFclModelToJson1(std::vector<fcl::CollisionObject<double>*> fclCollisionObjs, int pointCnt)
{
	std::vector<fcl::Vector3d> p;
	std::vector<fcl::Triangle> t;
	fcl::test::loadOBJFile("D:\\DDcode\\110kv-220kv-3d\\3DModel\\mesh\\cuboid.obj", p, t);
	std::shared_ptr<fcl::BVHModel<fcl::OBBRSSd>> geom = std::make_shared<fcl::BVHModel<fcl::OBBRSSd>>();
	geom->beginModel();
	geom->addSubModel(p, t);
	geom->endModel();
	fcl::CollisionObjectd* cuboid_t = new fcl::CollisionObjectd(geom);
	...
	for (int j = 0; j < fclCollisionObjs.size(); j++) {
		...
		for (int i = 0; i < pointCnt; i++) {
			...
			cv[1] = rand() % 20001 - 10000;
			cv[2] = rand() % 20001 - 10000;
			cuboid_t->setTranslation(cv);

			double dist = fcl::distance<double>(fclCollisionObjs[j], cuboid_t, request, result);
			if (dist >= 0) {
				arr.push_back("[" + std::to_string(result.nearest_points[0][0]) + "," + std::to_string(result.nearest_points[0][1]) + "," + std::to_string(result.nearest_points[0][2]) + "],");
			}
		}
